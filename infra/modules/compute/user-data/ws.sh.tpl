#!/bin/bash
# ChessVerse WebSocket — EC2 Boot Script
set -euxo pipefail
...
# Read the deployed image tag from SSM at boot — same as backend.
...
apt-get update -y
apt-get install -y docker.io awscli jq curl
        ...
        "collect_list": [
          {
            ...
            "log_stream_name": "{instance_id}/ws",
...
aws ecr get-login-password --region "$REGION" \
  | docker login --username AWS --password-stdin "$(echo $ECR_WS | cut -d'/' -f1)"

cat > /opt/chessverse/docker-compose.yml << DC_EOF
services:
  ws:
    image: $ECR_WS:$IMAGE_TAG
    restart: always
    ports:
      - "8080:8080"
...
aws ec2 create-tags --region "$REGION" --resources "$INSTANCE_ID" \
  --tags "Key=ChessverseService,Value=ws" "Key=Environment,Value=$ENV"

cd /opt/chessverse
docker compose pull && docker compose up -d
echo "ChessVerse ws started on $(date)"
